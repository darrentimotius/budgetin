<div x-data="{saveProfile(){
    ...
    <div class="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h4 class="text-lg font-semibold text-gray-800 dark:text-white/90 lg:mb-6">{{ __('common.address') }}</h4>

                <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
                    <div>
                        <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">{{ __('common.country') }}</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ auth()->user()->profile?->country ?? '-' }}</p>
                    </div>

                    <div>
                        <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">{{ __('common.city') }}</p>
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ auth()->user()->profile?->postal_code ?? '-' }}</p>
                    </div>
                </div>
            </div>
        ...
        <div
            ...
            <div class="px-2 pr-14">
                <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
                    {{ __('common.edit_address_info') }}
                </h4>
                <p class="mb-2 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                    {{ __('common.edit_address_info_description') }}
                </p>
            </div>
            <form class="flex flex-col overflow-y-auto" method="POST" action="{{ route('profile.update-address-information') }}">
                @csrf
                @method('POST')
